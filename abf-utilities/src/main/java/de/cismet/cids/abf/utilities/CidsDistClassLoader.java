/*
 * CidsDistClassLoader.java, encoding: UTF-8
 *
 * Copyright (C) by:
 *
 *----------------------------
 ...
 *----------------------------
 *
 * Created on 10. Dezember 2007, 11:27
 *
 */
...
/**
 * This Class is able the load all classes of libraries in a cidsDistribution.
 * Additionally it has a class cache to check if a class can generally be loaded
 * or if the class can be used in a given library context.
 *
 * @author mscholl
 */
public final class CidsDistClassLoader extends URLClassLoader
{
    private static final transient Logger LOG = Logger.getLogger(
            CidsDistClassLoader.class);

    private static final transient
            Map<FileObject, CidsDistClassLoader> INSTANCES =
    ...
    /**
     * Initializes the ClassLoader by loading all jars of the corresponding 
     * cidsDistribution
     */
    private void init()
    {
        ...
        if(srcFO == null)
        {
            LOG.warn("could not locate int dir, int " // NOI18N
                    + "libraries not loaded"); // NOI18N
        }else
        {
            loadJars(srcFO);
        }
        ...
        if(srcFO == null)
        {
            LOG.warn("could not locate ext dir, ext " // NOI18N
                    + "libraries not loaded"); // NOI18N
        }else
        {
            loadJars(srcFO);
        ...
        if(srcFO == null)
        {
            LOG.warn("could not locate local dir, local " // NOI18N
                    + "libraries not loaded"); // NOI18N
        }else
        {
            loadJars(srcFO);
    ...
    /**
     * Loads the classes of all jars in the given library dir into memory of the
     * ClassLoader
     *
    ...
    private void loadJars(final FileObject srcFO)
    {
        for(final Enumeration e = srcFO.getData(false); e.hasMoreElements();)
        {
            ...
            if(fo.getExt().equals("jar") || fo.getExt().equals("zip")) // NOI18N
            {
                ...
                {
                    LOG.error("could not add file " // NOI18N
                            + fo.getNameExt(), ex);
                }
            }
        }
    ...
    private void addToCache(final FileObject srcFO)
    {
        ...
        {
            final String message = "could not open jar file: " // NOI18N
                    + srcFile.getPath();
        ...
        for(final Enumeration<JarEntry> e = jar.entries(); e.hasMoreElements();)
        {
            ...
            {
                final String binaryName = entry.getName()
                        .replace('/', '.'); // NOI18N
                list.add(binaryName);
            }
        }
     ...
     * @param jarClassPath all absolute paths to jar/zip files that will be in
     *          the class path used by the application or null if a general
     *          check shall be performed where all libraries will be searched
    ...
    /**
     * Computes the hits for a given partial String of a class name. 
     * case-sensitive
     *
     * @param part any String
     * @return a HashMap containing all names of jars that contain classes, that
     *      contain the String part, as keys and the binary class names as 
     *      values
     */
    public Map<String, String[]> getHits(final String part)
    {
        this.part = part;
        if("".equals(part)) // NOI18N
        {
            return classCache;
        }
        computeHits();
    ...
    /**
     * Checks whether a given class could be found and loaded by this loader
     * instance.
     * 
     * @param name binary name of the class that shall be check for loadability
     * @param jarClassPath all absolute paths to jar/zip files that will be in
     ...
     *          check shall be performed where all libraries will be searched
     *          for the given class name
     * @return true if class was found in any library (in the application 
     *      context, if jarClassPath was not null), false otherwise
     */
    public boolean isLoadable(final String name, final String[] jarClassPath)
    {
        ...
        for(final String jar : jcp)
        {
            ...
            if(classes == null)
            {
                LOG.warn("you provided a jar in the jarClassPath " // NOI18N
                        + "array that could not be found in the " // NOI18N
     ...
     * @return the new classloader or null if the given cl is not managed by 
     *          this class (what should not happen though)
     */
    public CidsDistClassLoader fullReload(final CidsDistClassLoader loader)
    {
        for(final FileObject fo : INSTANCES.keySet())
        {
            ...
            {
                final CidsDistClassLoader newLoader = new CidsDistClassLoader(
                        fo);
                INSTANCES.remove(fo);
    ...
    /**
     * Creates a new instance of CidsDistClassLoader if it does not exist for 
     * the given cidsDistribution dir or returns the existing one.
     * 
    ...
    public static synchronized CidsDistClassLoader getInstance(final FileObject 
            distRoot)
    {
    ...
    /**
     * Checks whether a CidsDistClassLoader instance has already been created 
     * for the given cidsDistribution dir
     *
     * @param distRoot the FileObject pointing to the "cidsDistribution" dir
