/*
 * NewUserWizardAction.java, encoding: UTF-8
 *
 * Copyright (C) by:
 *
 *----------------------------
...
public final class NewUserWizardAction extends CookieAction 
{
    ...
    protected void performAction(final Node[] nodes)
    {
        ...
        wizard.setTitle("Neuen Benutzer anlegen");
        ...
        if(!cancelled)
        {
            ...
            final List<UserGroup> ugs = (List<UserGroup>)wizard.getProperty(
                    USERGROUP_PROP);
            final Backend backend = cookie.getDomainserverProject().
            ...
            {
                LOG.error("could not store new user: " // NOI18N
                        + newUser.getLoginname(), e);
                ErrorManager.getDefault().notify(e);
            }finally
            {
                cookie.getDomainserverProject().getLookup()
                        .lookup(UserManagement.class).refreshChildren();
            }
        }
    }
