...
public final class ImportClassesAction extends CookieAction {
    ...
    private boolean importClasses(final Backend backend,
        ...
        /*
         * create a cidsClass for each cs_class node in the import doucment and
         * add them to the classes list
         */
        for (int i = 0; i < classlist.getLength(); ++i) {
            final Node node = classlist.item(i);
            classes.add(createClass(node, refcache, typecache, io.getOut(), handle, " ", attrCache, classAttrCache, foreignKeyCache)); // NOI18N
         ...
         */
        ArrayList<CidsClass> storedCidsClasses = new ArrayList<CidsClass>();
        for (CidsClass cs : classes) {
         ...
         */
        for (CidsClass cs : storedCidsClasses) {
     ...
     * @return  DOCUMENT ME!
     *
     * if an idref or cs_type node appears, this method will return the
     * corresponding reference node in document, this means the node with the same id
     */
    private Node resolveNode(final Document doc, Class<? extends CommonEntity> which, Integer id) {
        //make a list with all nodes which could contain the referenced id
        ArrayList<NodeList> allNodeLists = new ArrayList<NodeList>();
        ...
        for (NodeList nodelist : allNodeLists) {
            if (stop) {
                break;
            }
            for (int j = 0; j < nodelist.getLength(); j++) {
                ...
                NodeList idList = nodelist.item(j).getChildNodes();
                for (int k = 0; k < idList.getLength(); k++) {
                    Node tmpChild = idList.item(k);
                    if (Node.ELEMENT_NODE == tmpChild.getNodeType()) {
                        String name = tmpChild.getLocalName();
                        String text = tmpChild.getTextContent();
    ...
    private <T extends CommonEntity> T resolveType(final Node node,
        ...
        if (cachedEntity == null) {
            ...
            for (final CommonEntity ce : systemTypes) {
                if (CidsClass.class.isAssignableFrom(which)) {
                    ...
                    if (comp.equalsIgnoreCase(((Policy) ce).getName())) {
                        out.println(NbBundle.getMessage(ImportClassesAction.class, "ImportClassesAction.createClass(Node,Map,Map,PrintWriter,ProgressHandle,String,Map,Map,Map).entityAllreadyExistsMessage",
                                PROCESS, indent));
                        return (T) ce;
                    }
                }
            ...
            /*
             * now we know, that the element doesnt exists yet.
             * we create it.
             */
            if (ID_REF.equals(child.getLocalName())) {

                String parentName = rightParent.getLocalName();
    ...
    private CidsClass createClass(
        ...
        for (int i = 0; i < children.getLength(); ++i) {
            ...
            if ((Node.ELEMENT_NODE == n.getNodeType()) && (NNAME.equals(n.getLocalName()) || ID_REF.equals(n.getLocalName()))) {
        ...
        if (cidsClass == null) {
            ...
            /*
             * first create the type, because it could be used in attributes of other classes
             * and so prevents infinite loops
             */
            ...
            for (int i = 0; i < children.getLength(); ++i) {
                ...
                if (Node.ELEMENT_NODE == n.getNodeType()) {
                    ...
                    if (ID.equals(localname)) {
                        ...
                        if (attr == null) {
                            attr = createAttribute(n, refcache, typecache, out, indent, attrCache, classAttrCache, foreignKeyCache);
                        ...
                        if (csAttr == null) {
                            csAttr = createClassAttribute(n, refcache, typecache, out, indent, attrCache, classAttrCache, foreignKeyCache);
    ...
    public Icon createIcon(final Node n, final Map<Class<? extends CommonEntity>, Map<Integer, ? extends CommonEntity>> refcache,
            final Map<Class<? extends CommonEntity>, List<? extends CommonEntity>> typecache,
            final PrintWriter out,
            final String preIndent) {
    ...
    public JavaClass createJavaClass(final Node n,
        ...
        final String indent = preIndent + "\t"; // NOI18N
        final Hashtable<Integer, JavaClass> javaClassesList = (Hashtable<Integer, JavaClass>) refcache.get(JavaClass.class);
            ...
            if (Node.ELEMENT_NODE == children.item(i).getNodeType()) {
                ...
                if (ID.equals(name)) {
                    ...
                    out.println(NbBundle.getMessage(
                            ...
                            "ImportClassesAction.createClass(Node,Map,Map,PrintWriter,ProgressHandle,String,Map,Map,Map).createEditorMessage",
                            CREATE, preIndent));
                    JavaClass jc = createJavaClass(children.item(i), refcache, typecache, out, preIndent);
                    type.setEditor(jc);
    ...
    public Attribute createAttribute(final Node n,
            ...
            final Map<String, Set<Attribute>> attrCache,
            final Map<CidsClass, Set<ClassAttribute>> classAttrCache,
            Map<Integer, CidsClass> foreignKeyCache) {

        final String indent = preIndent + "\t"; // NOI18N
        final Hashtable<Integer, Attribute> attributeList = (Hashtable<Integer, Attribute>) refcache.get(Attribute.class);
        ...
        for (int i = 0; i
            ...
            if (Node.ELEMENT_NODE == children.item(i).getNodeType()) {
                if (ID.equals(name)) {
                            ...
                            if (Node.ELEMENT_NODE == grandChilds.item(i).getNodeType()) {
                                ...
                                if (COMPLEX_TYPE.equals(grandChildName)) {
                                    if (Boolean.valueOf(grandChildText)) {
                                        /*
                                         * if the Type of the attribute is a complex one it
                                         * must be a cidsClass that wasn't created yet.
                                         * So we look for the right cs_class node and
                                         * create the cidsClass.
                                         */
                                        Node idNode = children.item(i).getFirstChild();
                                        String classIdValue = null;
                                        while (idNode.getNextSibling() != null) {
                                            if (Node.ELEMENT_NODE == idNode.getNodeType() && CLASS_ID.equals(idNode.getNodeName())) {
                                        ...
                                        final Node rightParent = resolveNode(children.item(i).getOwnerDocument(), CidsClass.class, Integer.valueOf(classIdValue));
                                        final CidsClass cidsClass = createClass(rightParent, refcache, typecache, out, null, preIndent, attrCache, classAttrCache, foreignKeyCache);
                                        type = cidsClass.getType();
                                    } else {
                                        /*
                                         * if it is not a complex type, and we could not
                                         * find in ref or typecache, just create a new type
                                         */
                    ...
                    CidsClass cidsClass = resolveType(children.item(i), CidsClass.class, refcache, typecache, out, preIndent);
                    if (cidsClass == null) {
                        final Node rightParent = resolveNode(children.item(i).getOwnerDocument(), CidsClass.class, Integer.valueOf(text));
                        cidsClass = createClass(rightParent, refcache, typecache, out, null, preIndent, attrCache, classAttrCache, foreignKeyCache);
                    }
    ...
    public ClassAttribute createClassAttribute(final Node n,
        ...
        final String indent = preIndent + "\t"; // NOI18N
        final Hashtable<Integer, ClassAttribute> csAttrList = (Hashtable<Integer, ClassAttribute>) refcache.get(ClassAttribute.class);
        ClassAttribute csAttr = new ClassAttribute();
        NodeList children = n.getChildNodes();
        Integer id = null;
                            ...
                            if (Node.ELEMENT_NODE == grandChilds.item(i).getNodeType()) {
                                ...
                                if (COMPLEX_TYPE.equals(grandChildName)) {
                                    if (Boolean.valueOf(grandChildText)) {
                                        ...
                                        /*
                                         * if it is not a complex type, and it wasn't
                                         * find in ref or typecache, just create a new type
                                         */
                                        type = createType(children.item(i), refcache, typecache, out, preIndent);
                                    }
