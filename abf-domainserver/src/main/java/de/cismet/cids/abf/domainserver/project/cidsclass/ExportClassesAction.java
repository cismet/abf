/*
 * ExportClassesAction.java, encoding: UTF-8
 *
 * Copyright (C) by:
 *
 *----------------------------
...
public final class ExportClassesAction extends CookieAction
{
    ...
    public static final String NAMESPACE =
            "http://cids.cismet.de/abf/cidsexport"; // NOI18N
    public static final String EXPORT_SCHEMA = "cids_export.xsd";
    public static final String FILECHOOSER_DIR =
            "Filechooser.importAndExportDirectory"; // NOI18N
    ...
    public static final String FOREIGN_KEY_REF =
            "foreign_key_references_to"; // NOI18N
    // DESCR
    ...
    protected void performAction(final org.openide.nodes.Node[] nodes)
    {
        ...
        final File outDir = new File(p.getProperty(
                FILECHOOSER_DIR,
        ...
        int ret = chooser.showSaveDialog(
        ...
        if(JFileChooser.APPROVE_OPTION == ret)
        {
            ...
            p.setProperty(
                    FILECHOOSER_DIR, outFile.getParentFile().getAbsolutePath());
            final List<CidsClass> classes = new LinkedList<CidsClass>();
            for(org.openide.nodes.Node n : nodes)
            {
                final CidsClassContextCookie cookie = n.getCookie(
            ...
            final InputOutput io, final ProgressHandle handle)
    {
        final Document doc = XMLUtil.createDocument(
                "class_export", NAMESPACE, null, null); // NOI18N
        ...
        try
        {
            ...
            for(final CidsClass clazz : classes)
            {
                doc.getDocumentElement().appendChild(createClassNode(
                        clazz, doc, refList, io.getOut(), " ")); // NOI18N
            ...
            final Document in  = XMLUtil.parse(
                    is, false, true, null, new EntityResolver()
            {
                @Override
            ...
            final String preIndent)
    {
        ...
        }else
        {
            ...
            classNode.appendChild(createNode(
                    NNAME, cidsClass.getName(), doc, out, indent));
            if(cidsClass.getDescription() != null)
            {
                // create desc node
                classNode.appendChild(createNode(
                        DESCR, cidsClass.getDescription(), doc, out, indent));
            }
            ...
            classNode.appendChild(createNode(
                    TABLE_NAME, cidsClass.getTableName(), doc, out, indent));
            ...
            classNode.appendChild(createPolicyNode(
                    POLICY, cidsClass.getPolicy(), doc, refList, out, indent));
            ...
            classNode.appendChild(createTypeNode(
                    cidsClass.getType(), doc, refList, out, indent));
            // create attribute nodes
            if(cidsClass.getAttributes() != null)
            {
                for(final Attribute attr : cidsClass.getAttributes())
                {
                    classNode.appendChild(createAttributeNode(
                            attr, doc, refList, out, indent));
                }
            }
            ...
            final String preIndent)
    {
        ...
        }else
        {
            ...
            iconNode.appendChild(createNode(
                    NNAME, icon.getName(), doc, out, indent));
            // icon file name
            iconNode.appendChild(createNode(
                    FILE_NAME, icon.getFileName(), doc, out, indent));
        ...
        }else
        {
            ...
            if(javaClass.getQualifier() != null)
            {
                jcNode.appendChild(createNode(
                        QUALIFIER, javaClass.getQualifier(), doc, out, indent));
            }
            // create type node
            jcNode.appendChild(createNode(
                    TYPE, javaClass.getType(), doc, out, indent));
            // create notice node
            if(javaClass.getNotice() != null)
            {
                jcNode.appendChild(createNode(
                    NOTICE, javaClass.getNotice(), doc, out, indent));
            }
            // add to reference list
            ...
            final String preIndent)
    {
        ...
        }else
        {
            ...
            pNode.appendChild(createNode(
                    NNAME, policy.getName(), doc, out, indent));
            // add to reference list
            ...
            final String preIndent)
    {
        ...
        }else
        {
            ...
            attrNode.appendChild(createTypeNode(
                    attr.getType(), doc, refList, out, indent));
            // create attr name node
            attrNode.appendChild(createNode(
                    NNAME, attr.getName(), doc, out, indent));
            // create attr field_name node
            attrNode.appendChild(createNode(
                    FIELD_NAME, attr.getFieldName(), doc, out, indent));
            ...
            {
                attrNode.appendChild(createNode(
                        DESCR, attr.getDescription(), doc, out, indent));
            }
            // create attr visible node
            attrNode.appendChild(createNode(
                    VISIBLE, attr.isVisible().toString(), doc, out, indent));
            // create attr indexed node
            attrNode.appendChild(createNode(
                    INDEXED, attr.isIndexed().toString(), doc, out, indent));
            // create attr isarray node
            attrNode.appendChild(createNode(
                    ISARRAY, attr.isArray().toString(), doc, out, indent));
            // create attr array_key node
            if(attr.isArray() && attr.getArrayKey() != null)
            {
                attrNode.appendChild(createNode(
                        ARRAY_KEY, attr.getArrayKey(), doc, out, indent));
            }
            // create attr editor node
            if(attr.getEditor() != null)
            {
                attrNode.appendChild(createJavaClass(
                        EDITOR, attr.getEditor(), doc, refList, out, indent));
            }
            ...
            attrNode.appendChild(createNode(
                    OPTIONAL, attr.isOptional().toString(), doc, out, indent));
            // create attr default value node
            if(attr.getDefaultValue() != null)
            {
                attrNode.appendChild(createNode(
                        DEFAULT_VAL, attr.getDefaultValue(), doc, out, indent));
            }
            ...
            {
                attrNode.appendChild(createNode(
                        POS, attr.getPosition().toString(), doc, out, indent));
            }
            ...
            final String preIndent)
    {
        ...
        }else
        {
            ...
            caNode.appendChild(createNode(
                    ATTR_KEY, attr.getAttrKey(), doc, out, indent));
            // create class attr_value node
            caNode.appendChild(createNode(
                    ATTR_VALUE, attr.getAttrValue(), doc, out, indent));
            // add to reference list
            ...
            final String preIndent)
    {
                ...
                && !refList.contains(CS_CLASS + type.getCidsClass().getId()))
        {
            ...
            doc.getDocumentElement().appendChild(createClassNode(
                    type.getCidsClass(), doc, refList, out, preIndent));
        }
        ...
        }else
        {
            ...
            typeNode.appendChild(createNode(
                    NNAME, type.getName(), doc, out, indent));
            ...
            {
                typeNode.appendChild(createNode(
                        DESCR, type.getDescription(), doc, out, indent));
            }
            // create type editor node
            if(type.getEditor() != null)
            {
                typeNode.appendChild(createJavaClass(
                        EDITOR, type.getEditor(), doc, refList, out, indent));
            }
