/*
 * PackageFolderNode.java, encoding: UTF-8
 *
 * Copyright (C) by:
 *
 *----------------------------
 ...
 *----------------------------
 *
 * Created on 14. August 2007, 16:34
 *
 */
...
public final class PackageFolderNode extends FilterNode implements Observer
{
    ...
    static
    {
        IMG_PACKAGE = ImageUtilities.loadImage(LibrarySupportProject
                .IMAGE_FOLDER + "package_16.png"); // NOI18N
        IMG_PACKAGE_EMTPY = ImageUtilities.loadImage(LibrarySupportProject
                .IMAGE_FOLDER + "packageEmpty_16.gif"); // NOI18N
    }
        ...
        super(folder.getNodeDelegate(),
                ...
                new ProxyLookup(new Lookup[]
                {
                    ...
                    Lookups.singleton(
                            ...
                            root.getPrimaryFile(), folder.getPrimaryFile())),
                }));
            ...
            final int index)
    {
            ...
            try
            {
                ...
                if(obj != null)
                {
                    ...
                    if(!DnDUtils.acceptFiles(
                            (List)obj, FileUtil.toFile(folder), action))
                    {
        ...
        }else if(t.isDataFlavorSupported(DnDUtils.URI_LIST_FLAVOR))
        {
            ...
            {
                if(DnDUtils
                        .acceptFiles(fileList, FileUtil.toFile(folder), action))
                {
                    return new UnifiedDnDFilePasteType(t, action, folder);
                }
            }else
    ...
    public Action[] getActions(final boolean context)
    {
        ...
        final ArrayList<Action> actions =
                new ArrayList<Action>(superActions.length + 7);
        ...
        for(final Action action : superActions)
        {
            ...
            if(action instanceof DeleteAction)
            {
                skip = true;
                actions.add(action);
                actions.add(CallableSystemAction
                        .get(RenamePackageWizardAction1.class));
