...
public final class RebuildFromJarAction extends CookieAction implements Cancellable {
    ...
    private final transient ThreadLocal<LinkedList<Node>> errorNodes = new ThreadLocal<LinkedList<Node>>();

    private transient ProgressHandle handle;

    private transient volatile int threadCount = 0;
    private transient volatile int handleSteps = 0;
    private transient volatile int handleStepsDone = 0;
    ...
    private void init() {
        ...
        if (++threadCount == 1) {
            handle = ProgressHandleFactory.createHandle(
                    org.openide.util.NbBundle.getMessage(
                        RebuildFromJarAction.class,
                        "RebuildFromJarAction.handle.message"),
                    (Cancellable)this); // NOI18N
    ...
    private void rebuildFromLocal(final Node[] nodes) {
        for (final Node node : nodes) {
            final LocalManagement local = node.getCookie(LocalManagementContextCookie.class)
    ...
    private void rebuildFromJar(final Node[] nodes) {
        ...
        for (final Node node : nodes) {
            final InputOutput ioTab = IOProvider.getDefault()
                        .getIO(
                            org.openide.util.NbBundle.getMessage(
                                RebuildFromJarAction.class,
                                "RebuildFromJarAction.rebuildFromJar(Node[]).iotab.message")
            ...
            try {
                ...
                out.println(
                    org.openide.util.NbBundle.getMessage(
                                RebuildFromJarAction.class,
                                "RebuildFromJarAction.rebuildFromJar(Node[]).out.workFolder") // NOI18N
                            + sourceDir.getPath());
                if (cancelAction) {
                    return;
                }
                ...
                out.println(
                    org.openide.util.NbBundle.getMessage(
                                RebuildFromJarAction.class,
                                "RebuildFromJarAction.rebuildFromJar(Node[]).out.sourceFolder") // NOI18N
                            + jarDir.getPath());
                if (cancelAction) {
                    return;
                }
                ...
                out.println(
                    org.openide.util.NbBundle.getMessage(
                                RebuildFromJarAction.class,
                                "RebuildFromJarAction.rebuildFromJar(Node[]).out.backupFolder") // NOI18N
                            + backupDir.getPath());
                if (cancelAction) {
                    return;
                }
                ...
                out.println(
                    org.openide.util.NbBundle.getMessage(
                                RebuildFromJarAction.class,
                                "RebuildFromJarAction.rebuildFromJar(Node[]).out.sourceArchive") // NOI18N
                            + jar.getPath());
                out.println();
                if (cancelAction) {
                    return;
                ...
                out.print(org.openide.util.NbBundle.getMessage(
                        RebuildFromJarAction.class,
                        "RebuildFromJarAction.rebuildFromJar(Node[]).out.doBackup")); // NOI18N
                doBackup(backupDir, sourceDir);
                out.println(org.openide.util.NbBundle.getMessage(
                        RebuildFromJarAction.class,
                        "RebuildFromJarAction.rebuildFromJar(Node[]).out.finished")); // NOI18N
                if (cancelAction) {
                    return;
                }
                out.print(org.openide.util.NbBundle.getMessage(
    ...
    private void printSuccess(final PrintWriter out, final long durationSec) {
        final String rebuildSuccess = org.openide.util.NbBundle.getMessage(
                RebuildFromJarAction.class,
                "RebuildFromJarAction.printSuccess(PrintWriter,long).rebuildSuccess"); // NOI18N
        final String duration = org.openide.util.NbBundle.getMessage(
                RebuildFromJarAction.class,
                "RebuildFromJarAction.printSuccess(PrintWriter,long).duration");       // NOI18N
        final String seconds = org.openide.util.NbBundle.getMessage(
                RebuildFromJarAction.class,
                "RebuildFromJarAction.printSuccess(PrintWriter,long).seconds");        // NOI18N
        final String secValue = String.valueOf(durationSec);
        final StringBuffer secondLine = new StringBuffer(
                rebuildSuccess.length()
                        + 4);
    ...
    private void detach(final Node node) {
        final SourceContextCookie src = node.getCookie(SourceContextCookie.class);
    ...
    private void cleanModifications(final Node node) {
        try {
            final FileObject src = node.getCookie(SourceContextCookie.class).getSourceObject();
    ...
    private void displayErrors(final Collection<Node> nodes) {
        ...
        for (final Node node : nodes) {
            ...
            .append(node.getDisplayName()).append("</li>"); // NOI18N
        }
        htmlList.append("</ul>");                           // NOI18N
        EventQueue.invokeLater(new Runnable() {
                ...
                public void run() {
                    JOptionPane.showMessageDialog(
                        ...
                        org.openide.util.NbBundle.getMessage(
                            RebuildFromJarAction.class,
                            "RebuildFromJarAction.displayErrors(Collection<Node>).JoptionPane.messageDialog.message",
                            htmlList.toString()),                                                                    // NOI18N
                        org.openide.util.NbBundle.getMessage(
                            RebuildFromJarAction.class,
                            "RebuildFromJarAction.displayErrors(Collection<Node>).JoptionPane.messageDialog.title"), // NOI18N
     ...
     */
    private void doBackup(final FileObject backupDir, final FileObject sourceDir) throws IOException {
     ...
     * @throws  IOException  DOCUMENT ME!
     */
    private FileObject getBackupDir(final FileObject sourceDir) throws IOException {
        FileObject mainBackupDir = sourceDir.getParent().getFileObject(BACKUP_DIR_NAME);
     ...
     */
    private FileObject getSourceDir(final Node node) throws FileNotFoundException, IllegalStateException {
        final FileObject sourceDir = node.getCookie(SourceContextCookie.class).getSourceObject();
        if (!sourceDir.canRead()) {
            throw new IllegalStateException(
                "cannot read source dir: "
                        + sourceDir.getPath()); // NOI18N
    ...
    private FileObject getJarDir(final Node node) {
        final LocalManagementContextCookie lmcc = node.getParentNode().getCookie(LocalManagementContextCookie.class);
        if (lmcc == null) {
...
//            return smcc.getStarterManagementContext().getStarterObject();
     ...
     * DOCUMENT ME!
     *
     * @param   jarDir   DOCUMENT ME!
     * @param   jarName  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     *
     * @throws  FileNotFoundException  DOCUMENT ME!
     * @throws  IllegalStateException  DOCUMENT ME!
     */
    private FileObject getJar(final FileObject jarDir, final String jarName) throws FileNotFoundException,
