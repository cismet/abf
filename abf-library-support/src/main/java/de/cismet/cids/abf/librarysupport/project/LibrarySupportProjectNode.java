...
public final class LibrarySupportProjectNode extends AbstractNode implements RefreshCookie {
    ...
    /**
     * Creates a new LibrarySupportProjectNode object.
     *
                ...
                for (final Action a : node.getActions(false)) {
                    ...
                    if (a instanceof DeployChangedJarsAction) {
                        for (final Node ch : node.getChildren().getNodes()) {
                            ...
                            if (modStore.anyModifiedInContext(
                                            FileUtil.toFile(
...
final class LibrarySupportProjectNodeChildren extends Children.Keys<FileObject> {
    ...
    /**
     * Creates a new LibrarySupportProjectNodeChildren object.
     *
     * @param  project  DOCUMENT ME!
     */
    public LibrarySupportProjectNodeChildren(final LibrarySupportProject project) {
        this.project = project;
    ...
    private void createStructure(final List<FileObject> fos, final boolean lcl) {
        ...
        final Thread runner = new Thread(new Runnable() {
                    ...
                    @Override
                    public void run() {
                        final ArrayList<Thread> threads = new ArrayList<Thread>(fos.size());
    ...
    private final class StructureCreator implements Runnable {
        ...
        /**
         * Creates a new StructureCreator object.
         *
        ...
        @Override
        public void run() {
            final File srcDir = new File(FileUtil.toFile(dir),
                    local ? LocalManagement.SRC_DIR : StarterManagement.SRC_DIR);
            if (!srcDir.mkdirs()) {
            ...
            for (final Enumeration<? extends FileObject> e = dir.getData(
                            ...
                            || toExtract.getExt().equalsIgnoreCase("zip"))                     // NOI18N
                {
                    ...
                    final Element extract = buildDoc.createElement("unjar");                   // NOI18N
            ...
            final File outFile = new File(FileUtil.toFile(dir), dir.getName() + "_build.xml"); // NOI18N
            ...
            try {
                final ExecutorTask task = ActionUtils.runTarget(FileUtil.toFileObject(outFile),
         ...
         * DOCUMENT ME!
         *
         * @param  extractDir  DOCUMENT ME!
         */
        public void cleanDir(final FileObject extractDir) {
            extractDir.refresh();
            for (final Enumeration<? extends FileObject> e = extractDir.getFolders(true); e.hasMoreElements();) {
                final FileObject fo = e.nextElement();
        ...
        public boolean moveAndCleanDir(final FileObject extractDir) {
            extractDir.refresh();
            final FileObject plain = extractDir.getFileObject(StarterManagement.SRC_DIR);
            if (plain == null) {
            ...
            for (final Enumeration<? extends FileObject> e = plain.getFolders(
                ...
                if (manifest == null) {
                        ...
                        ErrorManager.getDefault()
                                .annotate(
                                    ...
                                    MessageFormat.format(
                                        java.util.ResourceBundle.getBundle(
                                            "de/cismet/cids/abf/"              // NOI18N
                                            + "librarysupport/project/Bundle") // NOI18N
                                        .getString(
                                            "LibrarySupportProjectNode.moveAndCleanDir(FileObject).emptyManifestError"),
                                        fo                                     // NOI18N
                                        .getName()));
                    }
                } else {
                    ...
                    for (final Enumeration<? extends FileObject> e2 = fo.getFolders(true); e2.hasMoreElements();) {
