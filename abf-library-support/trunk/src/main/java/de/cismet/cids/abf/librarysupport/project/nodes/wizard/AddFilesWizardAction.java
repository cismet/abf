/*
 * AddFilesWizardAction.java, encoding: UTF-8
 *
 * Copyright (C) by:
 *
 *----------------------------
...
public final class AddFilesWizardAction extends NodeAction
{
    ...
    {
        final PackageContextCookie cookie = (PackageContextCookie)nodes[0].
                getCookie(PackageContextCookie.class);
        final WizardDescriptor wizard = new WizardDescriptor(getPanels(cookie.
                getRootFolder(), cookie.getCurrentFolder()));
            ...
            EventQueue.invokeLater(new Runnable()
            {
                ...
                public void run()
                {
                    final FileObject destinationFO = PackageUtils.toFileObject(
                            cookie.getRootFolder(), pakkage);
                    FileLock destLock = null;
                    final HashMap<FileObject, Throwable> erroneous =
                            new HashMap<FileObject, Throwable>();
                    try
                    {
